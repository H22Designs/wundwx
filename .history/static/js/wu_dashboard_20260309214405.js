...
const tempVal = f => {
    if (f == null) return "—";
    return useCelsius ? toC(f).toFixed(1) : Number(f).toFixed(1);
};
...
function historyUrl() {
    ...
    if (customStart && customEnd) {
        ...
        return `${base}&start=${encodeURIComponent(s)}&end=${encodeURIComponent(e)}&limit=3000`;
    }
    return `${base}&hours=${currentHours}&limit=3000`;
}
